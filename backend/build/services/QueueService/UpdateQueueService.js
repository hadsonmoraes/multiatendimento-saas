        ...
        name: Yup.string()
            ...
            if (value) {
                const queueWithSameName = yield Queue_1.default.findOne({
                    where: { name: value, companyId: queueData.companyId, id: { [sequelize_1.Op.not]: queueId } }
                });
        ...
        color: Yup.string()
            ...
            if (value) {
                const queueWithSameColor = yield Queue_1.default.findOne({
                    where: { color: value, companyId: queueData.companyId, id: { [sequelize_1.Op.not]: queueId } }
                });
